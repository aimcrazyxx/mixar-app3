...
def test_upload_create_poll_uses_official_p1_v3_schema():
    ...
    def handler(request):
        ...
        if request.url.path.endswith("/files"):
            view = "front" if b'front.png' in request.content else "left"
            return httpx.Response(200, json={"code": 0, "data": {"file_token": f"file-{view}"}})
        ...
        return httpx.Response(200, json={
            "code": 0,
            "data": {"status": "success", "output": {"model_url": "https://cdn/model.glb"}},
        })

    client = TripoP1Client("secret", transport=httpx.MockTransport(handler))
    try:
        tokens = {
            "front": client.upload_image(b"front", "front.png"),
            "left": client.upload_image(b"left", "left.png"),
        }
        task_id = client.create_multiview_task(
            tokens, texture=False, pbr=True, face_limit=500, model_seed=7,
        )
...
def test_front_and_second_view_are_required():
    client = TripoP1Client("secret", transport=httpx.MockTransport(lambda request: None))
    try:
...
def test_poll_failure_is_user_visible():
    def handler(request):
        return httpx.Response(200, json={
            "code": 0, "data": {"status": "failed", "message": "bad views"},
        })
