...
from mixar.modules.common.secure_storage import get_secret, masked_preview, set_secret
...
from ...core.openai_compatible import (
    OpenAICompatibleConfig, OpenAICompatibleProvider, parse_custom_headers,
)
...
def _key_value(wm) -> str:
    field = 'byok_custom_api_key_visible' if wm.byok_custom_show_key else 'byok_custom_api_key'
    return getattr(wm, field, '').strip() or get_secret('openai_compatible_api_key')


def _config(wm, key: str) -> OpenAICompatibleConfig:
    return OpenAICompatibleConfig(
        base_url=wm.byok_custom_base_url, api_key=key,
        model=wm.byok_custom_model, timeout=wm.byok_custom_timeout,
        max_output_tokens=wm.byok_custom_max_output_tokens,
        temperature=wm.byok_custom_temperature if wm.byok_custom_use_temperature else None,
        top_p=wm.byok_custom_top_p if wm.byok_custom_use_top_p else None,
        reasoning_effort=(wm.byok_custom_reasoning_effort
                          if wm.byok_custom_reasoning_effort != 'NONE' else ''),
        custom_headers=parse_custom_headers(wm.byok_custom_headers),
        context_limit=wm.byok_custom_context_limit,
        tool_calling=wm.byok_custom_tool_calling, vision=wm.byok_custom_vision,
...
def start_request(wm, *, action: str):
    try:
        ...
        wm.byok_dialog_state = 'ERROR'; wm.byok_last_error = str(exc)
        return {'CANCELLED'}
    wm.byok_dialog_state = 'SAVING'; wm.byok_last_error = ''; _redraw()
    ...
    def _worker():
        ...
        try:
            ...
            if action == 'save' and key and not set_secret('openai_compatible_api_key', key):
                raise RuntimeError("The operating-system credential store rejected the API key")
            if action == 'save':
                stored = {name: getattr(config, name) for name in (
                    'base_url', 'model', 'timeout', 'max_output_tokens',
                    'temperature', 'top_p', 'reasoning_effort', 'custom_headers',
                    'context_limit', 'tool_calling', 'vision', 'streaming',
                    'endpoint_mode',
                )}
            ...
            models, success, error = None, False, str(exc)
        finally:
    ...
    threading.Thread(
        target=_worker, daemon=True, name=f"MixarCustomProvider-{action}",
    ).start()
    return {'FINISHED'}
...
def _done(action, config, success, models, error):
    ...
    if not success:
        wm.byok_dialog_state = 'ERROR'; wm.byok_last_error = error or "Provider request failed"
        _redraw(); return
    if action == 'models':
    ...
    if action == 'save':
        wm.byok_custom_enabled = True; wm.byok_custom_base_url = config.base_url
        wm.byok_is_active = True; wm.byok_current_provider = 'openai-compatible'
    ...
    wm.byok_dialog_state = 'IDLE'; wm.byok_last_error = ''; _redraw()


class MIXAR_BYOK_OT_custom_toggle_key(Operator):
    bl_idname = "mixar_byok.custom_toggle_key"; bl_label = "Show or hide API key"
    bl_options = {'INTERNAL'}
...
class MIXAR_BYOK_OT_custom_test(Operator):
    bl_idname = "mixar_byok.custom_test"; bl_label = "Test connection"
    bl_options = {'INTERNAL'}
    def execute(self, context):
...
class MIXAR_BYOK_OT_custom_fetch_models(Operator):
    bl_idname = "mixar_byok.custom_fetch_models"; bl_label = "Fetch model list"
    bl_options = {'INTERNAL'}
    def execute(self, context):
...
class MIXAR_BYOK_OT_custom_copy_debug(Operator):
    bl_idname = "mixar_byok.custom_copy_debug"; bl_label = "Copy sanitized debug report"
    bl_options = {'INTERNAL'}
    def execute(self, context):
        context.window_manager.clipboard = context.window_manager.byok_custom_debug_report
        self.report({'INFO'}, "Sanitized debug report copied")
        return {'FINISHED'}


classes = (
    MIXAR_BYOK_OT_custom_toggle_key, MIXAR_BYOK_OT_custom_test,
    MIXAR_BYOK_OT_custom_fetch_models, MIXAR_BYOK_OT_custom_copy_debug,
)
