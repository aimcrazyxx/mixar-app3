...
from .openai_compatible import OpenAICompatibleConfig, OpenAICompatibleProvider, parse_custom_headers
...
def _snapshot_config(wm) -> OpenAICompatibleConfig:
    ...
    return OpenAICompatibleConfig(
        ...
        reasoning_effort=wm.byok_custom_reasoning_effort if wm.byok_custom_reasoning_effort != "NONE" else "",
...
def _ui_history(scene) -> list[dict]:
    ...
    for msg in scene.mixie_chat_messages:
        if getattr(msg, "loader_visible", False) or str(getattr(msg, "bubble_id", "")).startswith("temp_"):
            continue
        text = str(getattr(msg, "text", "") or getattr(msg, "content", "") or "").strip()
        if not text:
            continue
...
def _execute_tool_sync(script: str, timeout: float, cancel_event) -> dict:
    ...
    def _run():
        try:
            ...
            holder["result"] = {"success": False, "error": f"{type(exc).__name__}: {exc}"}
        finally:
            done.set()
    ...
    return holder.get("result") or {"success": False, "error": "Tool returned no result"}
...
def start(scene, encoded_attachments=None, wire_message: str = "") -> tuple[bool, str]:
    ...
    if not get_secret("openai_compatible_api_key") and not config.custom_headers:
    ...
    _stream_buffers[scene_name] = ""

    existing = _session_histories.get(session_id)
    messages = list(existing) if existing else _ui_history(scene)
    if existing:
        latest = next((m for m in reversed(_ui_history(scene)) if m.get("role") == "user"), None)
        if latest:
    ...
    if config.vision and encoded_attachments:
        for index in range(len(messages) - 1, -1, -1):
            if messages[index].get("role") == "user":
                messages[index] = _with_images(messages[index], list(encoded_attachments))
                break
    ...
    def _worker():
        ...
        try:
            ...
            result = run_agent_loop(
                ...
                on_text_delta=lambda delta: _stream_delta(scene_name, delta),
            )
            _session_histories[session_id] = result.messages
            _finalize(scene_name, result.text, result.debug, None)
        except Exception as exc:
            _finalize(scene_name, "", {}, exc)
        finally:
...
def _stream_delta(scene_name: str, delta: str) -> None:
    """Coalesce provider text deltas into the optimistic agent bubble."""
    from mixar.modules.space_mixie_chat.core.main_thread_executor import run_on_main_thread

    _stream_buffers[scene_name] = _stream_buffers.get(scene_name, "") + str(delta)
    ...
    def _apply():
        ...
        if scene is None:
            return
        text = _stream_buffers.get(scene_name, "")
        for message in reversed(scene.mixie_chat_messages):
            if str(getattr(message, "bubble_id", "")).startswith(TEMP_PLACEHOLDER_PREFIX):
...
def _finalize(scene_name: str, text: str, debug: dict, error) -> None:
    from mixar.modules.space_mixie_chat.core.main_thread_executor import run_on_main_thread

    def _apply():
        import bpy
        from mixar.modules.space_mixie_chat.constants import SessionState, TEMP_PLACEHOLDER_PREFIX
        from mixar.modules.space_mixie_chat.core.executor import get_executor
        from mixar.modules.space_mixie_chat.core.message_helpers import add_agent_message
        from mixar.modules.space_mixie_chat.core.session import get_session_manager
        from mixar.modules.space_mixie_chat.core.ui_utils import redraw_chat_areas
        ...
        for index in range(len(scene.mixie_chat_messages) - 1, -1, -1):
            if str(getattr(scene.mixie_chat_messages[index], "bubble_id", "")).startswith(TEMP_PLACEHOLDER_PREFIX):
        ...
        _stream_buffers.pop(scene_name, None)
