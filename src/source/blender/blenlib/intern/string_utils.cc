 ...
 * \ingroup bli
 *
 * Mixar overlay of upstream `string_utils.cc`: the only change is
 * #BLI_string_split_name_number parsing its numeric suffix without
 * exceptions (see the comment there). Re-copy from upstream on a version
 * bump and re-apply that one hunk.
 */
...
StringRef BLI_string_split_name_number(const StringRef name_full, const char delim, int &r_number)
{
  ...
  if (delim_index < name_full.size() - 1) {
    ...
    /* Mixar: parse without exceptions. Upstream wraps `std::stoi` in a
     * try/catch, but a suffix wider than an int ("Panel_Vert_0.6699999999999999",
     * an agent-scripted float in an object name) still aborted the app on
     * file load with an uncaught `std::out_of_range` from libc++, so the
     * conversion must not throw at all. An overflowing suffix is treated the
     * way upstream's catch intended: no number, the whole string is the name. */
    int number = 0;
    const char *first = num_str.data();
    const char *last = first + num_str.size();
