                                    ...
                                    NodeLinkItem *item)
{
  ...
  if (node_prev && node_from != node_prev) {
    for (bNodeSocket &sock_prev : node_prev->inputs) {
      for (bNodeSocket &sock_from : node_from->inputs) {
        ...
        if (STREQ(sock_prev.identifier, sock_from.identifier) && sock_prev.type == sock_from.type)
        {
...
/* Mixar: internal helper node groups created by the layer/paint system
 * (per-material "Mixar ..." groups, "~yP ..." layer/mask/modifier groups,
 * "~yPL ..." library groups, "~TL ..." temp helpers). These must never show
 * up in the socket link menu, mirroring the Python Add > Group menu filter in
 * paint/ui/utils/hide_internal_node_groups.py. They can't use Blender's own
 * "." hide prefix because their names are referenced by the paint code and by
 * a bundled library .blend. */
                                               ...
                                               std::optional<NodeDeclaration> &r_node_decl)
{
  ...
  if (arg->node_type->type_legacy == NODE_GROUP) {
    for (bNodeTree &ngroup : arg->bmain->nodetrees) {
      const char *ngroup_name = BKE_id_name(ngroup.id);
      if (ngroup_name[0] == '.' || node_group_is_mixar_internal(ngroup_name)) {
