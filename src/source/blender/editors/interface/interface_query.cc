...
bool region_contains_point_px(const ARegion *region, const int xy[2])
{
  ...
  if (region->v2d.mask.xmin != region->v2d.mask.xmax) {
    ...
    /* MIXAR: a mask that is a strict interior band in Y (touching neither the
     * region's bottom nor its top) means the View2D occupies only a sub-band
     * of the region, with real buttons above AND below it — the Agent Bubble
     * island, whose single region hosts a card header, a scrolled transcript
     * band and a composer. There, a point outside the mask must stay valid
     * for button hit-testing or every control outside the band is dead. No
     * stock editor produces this shape: their masks always anchor to the
     * region edges except for scroller strips on at most one side per axis,
     * so this changes nothing for them. Scroller hot-zones stay excluded. */
    const bool mask_is_interior_band = (v2d->mask.ymin > 0) &&
                                       (v2d->mask.ymax < region->winy - 1);
    if (!mask_is_interior_band && !BLI_rcti_isect_pt(&v2d->mask, mx, my)) {
      return false;
    }
    if (view2d_mouse_in_scrollers(region, &region->v2d, xy)) {
      return false;
    }
  }
