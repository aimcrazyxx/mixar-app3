                         ...
                         const Button *active_but)
{
  ...
  /* MIXAR: the Mixie sidebar draws its own GPU category strip
   * (#UI_panel_category_draw_all_mixar) instead of upstream's tab buttons,
   * so tab clicks are hit-tested against the rects recorded at draw time.
   * See interface_mixar_section.cc. */
  {
    const ScrArea *area = CTX_wm_area(C);
    if (area && area->spacetype == SPACE_MIXIE && event->type == LEFTMOUSE &&
        event->val == KM_PRESS)
    {
      if (const char *mixar_category = UI_mixar_panel_category_find_at(region, event->mval)) {
        panel_category_active_set(region, mixar_category);
        ED_region_tag_redraw(region);
        return WM_UI_HANDLER_BREAK;
      }
    }
