...
bool ED_region_overlap_isect_any_xy(const ScrArea *area, const int event_xy[2])
{
  for (ARegion &region : area->regionbase) {
    ...
    /* Fixed-width Zen drawer: only the grip and the painted slice are an
                                       ...
                                       const int event_xy[2])
{
  ...
  for (ARegion &region : area->regionbase) {
    ...
    if (ELEM(regiontype, RGN_TYPE_ANY, region.regiontype)) {
      /* RIGHT-aligned overlap only clips Y, so a fixed-width overlay would
       * otherwise swallow the whole strip. The drawer owns the grip and the
