...
/* Mixar: Zen-mode gating for the persp/ortho button (see below). */
...
static void WIDGETGROUP_navigate_draw_prepare(const bContext *C, wmGizmoGroup *gzgroup)
{
  ...
  if (show_navigate) {
    ...
    if ((RV3D_LOCK_FLAGS(rv3d) & RV3D_LOCK_ROTATION) == 0) {
      ...
      /* Mixar: Zen mode drops the perspective/orthographic toggle — the
       * calm canvas keeps only zoom / move / camera in this cluster. The
       * projection is still switchable everywhere else it always was
       * (Numpad 5, View menu, the N-panel); only this button is hidden,
       * and only in the Zen workspace. Name-matched the same way
...
void VIEW3D_GGT_navigate(wmGizmoGroupType *gzgt)
{
  ...
  /* Mixar: no backdrop chrome behind the navigate cluster — the wireframe
   * globe in `view3d_gizmo_navigate_type.cc` is the whole mark. */
}
