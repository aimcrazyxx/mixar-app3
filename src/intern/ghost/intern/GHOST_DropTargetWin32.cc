                                                   ...
                                                   DWORD *pdw_effect)
{
  ...
  *pdw_effect = DROPEFFECT_NONE;
  ...
  /* Preview file paths during hover; the event owns this independent copy.
   * Do not request text/bitmap payloads until the user actually drops. */
  void *preview = dragged_object_type_ == GHOST_kDragnDropTypeFilenames ?
                      getDropDataAsFilenames(p_data_object) : nullptr;
  system_->pushDragDropEvent(
      GHOST_kEventDraggingEntered, dragged_object_type_, window_, pt.x, pt.y, preview);
  return S_OK;
}
