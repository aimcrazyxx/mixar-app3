...
#ifdef _WIN32
  /**
   * Mixar: DWM frost needs a non-opaque swapchain. Recreates the swapchain
   * with premultiplied (or post-multiplied) composite alpha when the surface
   * supports it. The main Mixar window stays opaque.
   */
  bool mixar_set_premultiplied_composite_alpha(bool enable);
  bool mixar_supports_non_opaque_composite_alpha() const;
#endif
...
#ifdef _WIN32
  bool mixar_premul_composite_alpha_ = false;
#endif
